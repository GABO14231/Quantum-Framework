export class QuantumSlider extends Quantum
{
    ...
    showSlide(index)
    {
        const slides = this.mainElement.querySelectorAll(".slide");
        if (slides.length === 0) return console.error("No slides found in the DOM.");

        slides.forEach(slide => slide.classList.remove("active", "incoming-left", "incoming-right",
            "outgoing-left", "outgoing-right"));

        const incomingSlide = slides[index % slides.length];
        if (index === 0) {if (incomingSlide) incomingSlide.classList.add("active");}
        else
        {
    ...
    prevSlide() {this.showSlide((this.currentIndex - 1) % this._images.length);}
    nextSlide() {this.showSlide((this.currentIndex + 1) % this._images.length);}

    updateArrows()
    {
        if (this.currentIndex === 0) this.leftArrow.classList.add("hidden");
        else
        {
            this.leftArrow.removeAttribute("disabled");
            this.leftArrow.classList.remove("hidden");
        }
        if (this.currentIndex === this._images.length - 1) this.rightArrow.classList.add("hidden");
        else
        {
